...
| File                               | Notes                                                                                                                                                                                                                                                                                                        |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `eslint.config.{m,c}js`            | Merges `baseConfig` from each template; includes `.gitignore` patterns. Under `CI=true`, the `eslint-plugin-prettier` plugin and every `prettier/*` rule are stripped from each merged config (Prettier already runs as its own check; running it again through ESLint's AST pipeline is redundant and slow) |
