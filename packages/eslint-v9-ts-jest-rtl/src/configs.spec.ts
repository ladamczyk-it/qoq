...
describe('defineConfig composition', () => {
  it('resolves spec files to the same effective config as the legacy merged baseConfig', async () => {
    const legacy = await resolveConfigForFile([baseConfig, tsFilesScope], 'src/example.spec.tsx');
    const composed = await resolveConfigForFile(
      [...configs.base, tsFilesScope],
  ...
  it('resolves non-spec files to the same effective config as the legacy merged baseConfig', async () => {
    const legacy = await resolveConfigForFile([baseConfig, tsFilesScope], 'src/example.ts');
    const composed = await resolveConfigForFile([...configs.base, tsFilesScope], 'src/example.ts');
  ...
  it('resolves sonarjs/no-incompatible-assertion-types as enabled for spec files', async () => {
    const composed = await resolveConfigForFile(
      [...configs.base, tsFilesScope],
      'src/example.spec.tsx'
    );

    expect(composed.rules['sonarjs/no-incompatible-assertion-types']?.[0]).not.toBe(0);
  });
});
